...
// The Claude Code copy is generated by `npm run toolkit:install`, which is how this
// repository consumes its own published toolkit. Drift here means the installer broke or
// someone hand-edited a generated file.
const claudeSkillPath = ".claude/skills/code-review/SKILL.md";
const claudeSkill = await load(claudeSkillPath);
if (skill !== claudeSkill) {
  errors.push(`${claudeSkillPath}: must match canonical ${skillPath}; re-run npm run toolkit:install`);
}
...
const mcpConfig = await load(mcpConfigPath);
for (const marker of ['"supabase"', '"cloudflare-bindings"', '"cloudflare-observability"']) {
  requireText(mcpConfigPath, mcpConfig, marker);
}
// A committed MCP config must never carry a credential or a private project reference.
for (const forbidden of ["project_ref", "access_token", "api_key", "account_id"]) {
  if (mcpConfig.includes(forbidden)) {
    errors.push(`${mcpConfigPath}: must not contain ${forbidden}; MCP servers authenticate interactively`);
  }
}
